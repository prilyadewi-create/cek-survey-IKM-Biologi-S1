...
def load_data():
    ...
    response_data['pilihan matkul'] = response_data['pilihan matkul'].str.strip().str.upper()
    ...
    if student_id:
        ...
        if not student_courses.empty:
            ...
            student_courses['IKM Sudah Terisi'] = student_courses['Matakuliah'].apply(
                lambda x: 'Sudah' if x in response_data_filtered['pilihan matkul'].values else 'Belum'
            )
